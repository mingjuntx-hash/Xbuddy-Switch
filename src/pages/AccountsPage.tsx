...
export default function AccountsPage() {
   ...
   *
   * 只在用户开着「添加账号后自动绑定」时执行；任何失败都只提示，不阻断导入。
   */
  async function autoRunNewbieFor(accountId: string) {
    const r = await api.runAutoNewbieForAccount(accountId);
    if (!r.ok) {
      toast.warning("新人礼包自动处理未完成", { description: r.error });
      return;
    }
    if (r.bind) {
      if (r.bind.ok) {
        toast.success("已自动绑定邀请码", {
          description: r.bind.message,
        });
      } else {
        toast.warning("邀请码未绑定", {
          description: r.bind.message || "该账号可能不是新注册用户",
        });
      }
    }
    if ((r.credit ?? 0) > 0) {
      toast.success(`已自动领取 ${r.credit} 积分`);
    }
  }
  ...
  return (
    <div className="mx-auto w-full max-w-[1180px] px-6 py-8 sm:px-8 sm:py-9">
      ...
      <NewbiePanel accountCount={accounts.length} />
