// xbuddy-switch postinstall：从「平台包」复制本平台二进制。
//
// 平台分包（esbuild 模式）：二进制发布在独立 npm 包（xbuddy-switch-<platform>-<arch>），
...
const PLATFORM_PKG = `xbuddy-switch-${process.platform}-${process.arch}`;

if (!FILE) {
  console.warn(
    `xbuddy-switch: 跳过平台 ${process.platform}-${process.arch}（当前不支持），` +
      `可手动下载二进制后放置到 bin/ 目录`,
  );
  process.exit(0);
}
...
function fail(msg) {
  console.error(`xbuddy-switch install: ${msg}`);
  console.error(
...
function copyFrom(src) {
  ...
  console.log(
    `xbuddy-switch: 二进制就绪 → ${target} (${(size / 1048576).toFixed(1)}MB)`,
  );
}
...
async function main() {
  ...
  // 2) 从平台包复制（node_modules/xbuddy-switch-<platform>-<arch>/bin/<file>）
  try {
    ...
    if (fs.existsSync(target)) {
      console.log("xbuddy-switch: 二进制已存在，跳过");
      return;
    }
    return fail(
