#!/usr/bin/env node
// xbuddy-switch npm 入口：spawn 平台二进制（postinstall 下载，见 scripts/install.js）。
...
if (!FILE) {
  console.error(`xbuddy-switch: 不支持平台 ${process.platform}-${process.arch}`);
  process.exit(1);
}
...
if (!fs.existsSync(binPath)) {
  console.error(
    "xbuddy-switch: 未找到平台二进制，请重新安装（npm install -g xbuddy-switch 触发下载）",
  );
  process.exit(1);
}
