...
/// 成长计划任务接口前缀（网页端 v2 体系）。
/// 列表：GET {GROWTH_API_PREFIX}/tasks
/// 接受：POST {GROWTH_API_PREFIX}/tasks/accept  body {"task_codes": [...]}
/// 领取：POST {GROWTH_API_PREFIX}/tasks/{task_code}/claim
pub const GROWTH_API_PREFIX: &str = "/v2/activity/growth";

/// 邀请活动接口前缀（邀请码绑定 / 我的邀请码 / 邀请进度）。
pub const INVITE_API_PREFIX: &str = "/activity/workbuddy/invitation/v2";

/// 新人默认邀请码（Ming 的邀请码）。用户可在设置里改成自己的。
pub const DEFAULT_INVITE_CODE: &str = "55wg4s35enef";

/// WorkBuddy 客户端的 CDP 调试端口（任务自动化用）。
///
/// 客户端需带 `--remote-debugging-port=<此端口>` 启动，否则连不上。
pub const DEFAULT_CDP_PORT: u16 = 9222;

/// 默认新人礼包积分数（仅用于界面展示「最高可得」）。
pub const DEFAULT_NEWBIE_CREDIT: i64 = 2000;
...
pub fn newbie_config_file() -> PathBuf {
    store_dir().join("newbie_config.json")
}

pub fn newbie_logs_file() -> PathBuf {
    store_dir().join("newbie_logs.json")
}
...
// ---------------------------------------------------------------------------
// 新人礼包配置（邀请码绑定 + 新手任务自动领取）
// ---------------------------------------------------------------------------

/// 默认新人礼包配置。
///
/// - `invite_code`：要绑定的邀请码；留空则退回 [`DEFAULT_INVITE_CODE`]。
/// - `invite_link`：用户可直接粘贴邀请链接，保存时自动解析出邀请码。
/// - `auto_bind_on_add`：添加新账号后自动绑定邀请码。
/// - `auto_accept_tasks`：自动接受成长计划任务。
/// - `auto_claim_tasks`：自动领取已完成任务的积分。
pub fn default_newbie_config() -> Value {
    json!({
        "enabled": true,
        "invite_code": DEFAULT_INVITE_CODE,
        "invite_link": "",
        "auto_bind_on_add": true,
...
/// 支持三种形态：
/// - 纯邀请码：`55wg4s35enef`
/// - 带参数链接：`https://www.workbuddy.cn/events/invite/?inviteCode=55wg4s35enef`
/// - 其它含 `inviteCode=` 的文本
pub fn parse_invite_code(input: &str) -> String {
    let text = input.trim();
    if text.is_empty() {
        return String::new();
    }
    // 优先从 inviteCode= 参数提取
    if let Some(pos) = text.find("inviteCode=") {
        let rest = &text[pos + "inviteCode=".len()..];
...
fn merge_newbie_config(input: &Value) -> Value {
    let mut merged = default_newbie_config();
    let Some(map) = input.as_object() else {
        return merged;
    };
    ...
    let resolved = if !link.is_empty() {
        let parsed = parse_invite_code(link);
        if parsed.is_empty() {
            parse_invite_code(code_field)
        } else {
            parsed
        }
    } else {
    ...
    merged["invite_link"] = json!(link);
    merged["invite_code"] = json!(if resolved.is_empty() {
...
/// 读取新人礼包配置（缺失/损坏时合并默认值）。
pub fn load_newbie_config() -> Value {
    let f = newbie_config_file();
    if f.exists() {
        if let Ok(text) = std::fs::read_to_string(&f) {
            if let Ok(value) = serde_json::from_str::<Value>(&text) {
                return merge_newbie_config(&value);
            }
        }
    }
    default_newbie_config()
}

/// 保存新人礼包配置（只保留已知字段）。
pub fn save_newbie_config(cfg: &Value) -> std::io::Result<Value> {
    let merged = merge_newbie_config(cfg);
    std::fs::create_dir_all(store_dir())?;
    let content = serde_json::to_string_pretty(&merged).unwrap_or_default();
    atomic_write(&newbie_config_file(), &content)?;
    Ok(merged)
}

/// 读取新人礼包执行日志。
pub fn load_newbie_logs() -> Vec<Value> {
    let f = newbie_logs_file();
    if f.exists() {
        if let Ok(text) = std::fs::read_to_string(&f) {
            if let Ok(Value::Array(arr)) = serde_json::from_str::<Value>(&text) {
                return arr;
...
/// 保存新人礼包执行日志（保留最近 200 条）。
pub fn save_newbie_logs(logs: &[Value]) -> std::io::Result<()> {
    let mut kept: Vec<Value> = logs.to_vec();
    if kept.len() > 200 {
        kept.drain(..kept.len() - 200);
    }
    std::fs::create_dir_all(store_dir())?;
    let content = serde_json::to_string_pretty(&kept).unwrap_or_default();
    atomic_write(&newbie_logs_file(), &content)
}

/// 追加一条新人礼包执行日志。
///
/// 这里是「读-改-写」，必须串行化：`run_newbie_all` 并发处理多账号时，
/// 否则多个账号的结果会互相覆盖、丢日志。
pub fn add_newbie_log(entry: &Value) {
    static LOCK: OnceLock<Mutex<()>> = OnceLock::new();
    let _guard = LOCK
        .get_or_init(|| Mutex::new(()))
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let mut logs = load_newbie_logs();
    logs.push(entry.clone());
    let _ = save_newbie_logs(&logs);
}
