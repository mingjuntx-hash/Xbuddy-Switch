...
pub(crate) fn is_self_image_name(name: &str) -> bool {
    ...
    stem.eq_ignore_ascii_case("workbuddy-switch")
        ...
        || stem.eq_ignore_ascii_case("xbuddy-switch")
}

/// 精确匹配 WorkBuddy / CodeBuddy 映像，禁止子串命中 workbuddy-switch / xbuddy-switch。
...
// - 排除自身 pid 与 args 含 wb-switch / workbuddy-switch / xbuddy-switch 的 PID（自排除）。
...
/// args 是否引用了本工具自身（用于从进程扫描结果里自排除）。
///
/// 大小写不敏感：macOS 上 app bundle 名是 `XBuddy-Switch.app`，而这里拿到的是
/// `ps` 的原样输出，不能假设小写；历史上二进制名是 `wb-switch*`，两种都要认。
///
/// 与平台无关，刻意不挂 `cfg(target_os = "macos")`，以便在任意平台跑单测。
#[cfg_attr(not(target_os = "macos"), allow(dead_code))]
pub(crate) fn args_mention_self(args: &str) -> bool {
    let args_lc = args.to_ascii_lowercase();
    args_lc.contains("wb-switch")
        || args_lc.contains("workbuddy-switch")
        || args_lc.contains("xbuddy-switch")
}
...
/// 过滤：排除自身 pid；排除 args 引用本工具自身的 PID（见 [`args_mention_self`]）。
...
mod tests {
    ...
    fn self_image_names_are_detected() {
        ...
        assert!(is_self_image_name("XBuddy-switch.exe"));
        assert!(is_self_image_name("xbuddy-switch"));
    ...
    #[test]
    fn args_mention_self_is_case_insensitive_and_covers_both_brands() {
        // 新品牌名：macOS app bundle 名带大写 S
        assert!(args_mention_self(
            "/Applications/XBuddy-Switch.app/Contents/MacOS/XBuddy-Switch"
        ));
        assert!(args_mention_self(
            "/Applications/XBuddy-Switch.app/Contents/MacOS/wb-switch-rust"
        ));
        // 旧品牌名：历史安装路径与二进制名
        assert!(args_mention_self(
            "/Applications/workbuddy-switch.app/Contents/MacOS/wb-switch"
        ));
        assert!(args_mention_self("/opt/tool/wb-switch/helper run"));
        assert!(args_mention_self(
            r"C:\Program Files\XBuddy-Switch\xbuddy-switch.exe"
        ));
        // 目标应用自身不能被误判为「本工具」
        assert!(!args_mention_self(
    ...
    fn workbuddy_image_name_is_exact_not_substring() {
        ...
        assert!(!is_workbuddy_image_name("xbuddy-switch.exe"));
        ...
        assert!(!is_workbuddy_exe_file_name(Path::new("xbuddy-switch.exe")));
...
"#;
    ...
    fn macos_ps_filter_matches_case_sensitively_and_excludes_self() {
        ...
        // 5006/5007 引用新品牌名的自家进程：app bundle 名是 XBuddy-Switch.app（含大写），
        //   自排除必须大小写不敏感才拦得住。
        let stdout = format!(
            "{self_pid} /Applications/workbuddy-switch.app/Contents/MacOS/wb-switch\n\
             ...
             5006 /Applications/XBuddy-Switch.app/Contents/MacOS/XBuddy-Switch\n\
             5007 /Applications/XBuddy-Switch.app/Contents/Resources/helper/xbuddy-switch-helper\n"
        );
        let patterns = macos_main_patterns(None);
