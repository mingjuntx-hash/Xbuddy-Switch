...
pub const GITHUB_OWNER: &str = "mingjuntx-hash";
pub const GITHUB_REPO: &str = "Xbuddy-Switch";

/// 历史仓库（迁移用）：旧账号下的仓库名，遇到应自动改写到当前仓库。
const LEGACY_OWNERS: &[&str] = &["changexbc"];
const LEGACY_REPOS: &[&str] = &["workbuddy-switch", "wb-switch"];
...
pub fn load_github_config() -> Value {
    ...
    // 历史仓库迁移：旧账号（changexbc）下的仓库统一改写到当前仓库。
    // 否则用户本地遗留的 github_config.json 会把更新检查带向已废弃的地址。
    if LEGACY_OWNERS.contains(&owner.as_str()) {
        owner = GITHUB_OWNER.to_string();
        if LEGACY_REPOS.contains(&repo.as_str()) {
...
mod tests {
    ...
    /// URL 前缀使用常量拼接：断言的是「去重规则」，仓库名变更时测试不需要跟着改。
    fn base() -> String {
    ...
    fn updater_manifest_urls_macos_skips_duplicate_fallback() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                format!("{}/latest-macos-aarch64.json", base()),
            ]
        );
    }
    ...
    fn updater_manifest_urls_windows_keeps_macos_compat() {
        ...
        assert_eq!(
            ...
            vec![
                format!("{}/latest.json", base()),
                format!("{}/latest-windows-x86_64.json", base()),
                format!("{}/latest-macos-x86_64.json", base()),
            ]
        );
    }
