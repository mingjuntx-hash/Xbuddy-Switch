...
/// GET /api/newbie/config —— 新人礼包配置。
#[tauri::command]
pub fn get_newbie_config() -> Value {
    crate::modules::config::load_newbie_config()
}

/// POST /api/newbie/config —— 保存新人礼包配置。
///
/// `invite_link` 里若粘贴了完整邀请链接，保存时自动解析出邀请码。
#[tauri::command]
pub fn save_newbie_config(config: Value) -> Result<Value, String> {
    crate::modules::config::save_newbie_config(&config).map_err(|e| e.to_string())
}

/// GET /api/newbie/status —— 单账号新人礼包状态（配置 + 邀请进度 + 任务概览）。
#[tauri::command]
pub async fn get_newbie_status(account_id: String) -> Result<Value, String> {
    let account = account::find_account(&account_id).ok_or("账号不存在")?;
    Ok(crate::modules::growth::newbie_status(&account).await)
}

/// POST /api/newbie/bind —— 给指定账号绑定邀请码（不传则用配置里的）。
#[tauri::command]
pub async fn bind_invite(account_id: String, invite_code: Option<String>) -> Result<Value, String> {
    let account = account::find_account(&account_id).ok_or("账号不存在")?;
    let code = invite_code.unwrap_or_else(|| {
        crate::modules::config::load_newbie_config()
            .get("invite_code")
    ...
    Ok(crate::modules::growth::bind_invite_code(&account, &code).await)
}

/// POST /api/newbie/run —— 对指定账号跑新人流程（绑定邀请码 + 接受任务 + 领取积分）。
#[tauri::command]
pub async fn run_newbie_for_account(
    account_id: String,
...
#[tauri::command]
pub async fn run_newbie_all(bind_invite: Option<bool>) -> Result<Value, String> {
    Ok(crate::modules::growth::run_newbie_all(bind_invite.unwrap_or(false)).await)
}

/// POST /api/newbie/claim-all —— 只做「接受任务 + 领取积分」，不碰邀请码。
#[tauri::command]
pub async fn claim_all_newbie_tasks() -> Result<Value, String> {
    Ok(crate::modules::growth::run_newbie_all(false).await)
}

/// GET /api/newbie/logs —— 新人礼包执行日志。
#[tauri::command]
pub fn get_newbie_logs() -> Value {
    json!({ "logs": crate::modules::config::load_newbie_logs() })
}
...
// ---------------------------------------------------------------------------
//
// 成长计划任务的「完成」只能由客户端里的真实操作产生（服务端按行为判定）。
// 这组命令通过 CDP 在客户端里真实输入并发送消息，补上 API 做不到的那一环。
//
// 前置：WorkBuddy 客户端需带 --remote-debugging-port=9222 启动。

/// GET /api/autotask/status —— CDP 连接状态 + 客户端当前账号 + 待办统计。
#[tauri::command]
pub async fn autotask_status() -> Value {
    crate::modules::autotask::status().await
}

/// GET /api/autotask/probe —— 诊断客户端 DOM（输入框 / 按钮结构）。
///
/// 排查用：不同版本客户端的输入框 DOM 可能变化，先看它再调选择器。
#[tauri::command]
pub async fn autotask_probe() -> Value {
    let port = crate::modules::config::DEFAULT_CDP_PORT;
    tauri::async_runtime::spawn_blocking(move || crate::modules::client_cdp::probe(port))
        .await
        .unwrap_or_else(|e| json!({"ok": false, "error": format!("诊断线程异常: {e}")}))
}

/// POST /api/autotask/run —— 自动完成客户端当前账号的可自动化任务。
#[tauri::command]
pub async fn autotask_run(only: Option<Vec<String>>, dry_run: Option<bool>) -> Value {
    crate::modules::autotask::run(only, dry_run.unwrap_or(false)).await
}
...
/// 用系统默认方式打开任意 URL —— 含 `workbuddy://` 这类自定义协议。
///
/// Tauri 的 opener 插件受 capabilities 白名单约束，打不开自定义协议
/// （报 `Not allowed to open url workbuddy://...`）。任务深链必须用
/// 系统 shell 拉起，所以这里单独开一个命令。
#[tauri::command]
pub fn open_external_url(url: String) -> Result<(), String> {
    ...
    // 白名单校验：只放行明确需要的协议，避免退化成任意命令执行。
    let lower = trimmed.to_ascii_lowercase();
    let allowed = lower.starts_with("http://")
        || lower.starts_with("https://")
        || lower.starts_with("workbuddy://")
        || lower.starts_with("codebuddy://")
        || lower.starts_with("xbuddy://");
    if !allowed {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        std::process::Command::new("cmd")
            .raw_arg(format!("/C start \"\" \"{trimmed}\""))
