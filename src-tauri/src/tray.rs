...
const DEFAULT_TOOLTIP: &str = "XBuddy-Switch";
...
mod tests {
    ...
    fn app_bundle_path_from_packaged_exe() {
        use std::path::Path;
        let exe = Path::new("/Applications/XBuddy-Switch.app/Contents/MacOS/wb-switch-rust");
        assert_eq!(
            super::app_bundle_path_from_exe(exe),
            Some(Path::new("/Applications/XBuddy-Switch.app"))
        );
    }
    ...
    fn app_bundle_path_none_outside_app_bundle() {
        ...
        assert!(super::app_bundle_path_from_exe(Path::new(
            "/Applications/XBuddy-Switch.app/Contents/Resources/icon.icns"
        ))
        .is_none());
    }
